...
const Card = ({ product, time }: { product: TProduct; time?: string }) => {
  ...
  return (
    <Link
      ...
      className=" w-full relative  space-y-2 text-black  md:hover:shadow-xl hover:duration-500  border-2 transition duration-500 ease-in-out"
    >
      <Image
      ...
      <p className=" absolute text-white top-0 right-2 font-bold bg bg-[#a2e233] px-6  rounded-tl-[50px] rounded-br-[50px]">
        {`${Math.round(((product.price - product.discountPrice) / 230) * 100)}`}
        %
      </p>
      <p className=" absolute text-white top-0 left-2 font-bold bg bg-[#a2e233] px-6  rounded-tl-[50px] rounded-br-[50px]">
        {time}
      </p>
    </Link>
  );
